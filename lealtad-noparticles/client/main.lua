...
USO: /panicmode (activa y desactiva el panicmode)]]
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
        local xpos, ypos, zpos, radio = 0.0, 0.0, 0.0, 5000000000000.0
...
function goCoords()
    SetPedCoordsKeepVehicle(PlayerPedId(), coords.x, coords.y, coords.z) -- Pretty weird to have a function for just one thing
end

RegisterCommand('panicmode', function() -- Recomiendo poner aqui para que vaya por permisos de admin (todos pueden usar este comando)
	if not panicMode then
		panicMode = true
